...
function regnUtBesparelseIProsent(original: number, nyStorrelse: number): number {
...
async function fetchFromUnsplashAndRunThroughSqip(query: string) {
    ...
    const tilfeldigTall = tilfeldigTallMellom(0, unsplashResponse?.response?.results.length ?? 0);
    log.success(
        `Fant ${unsplashResponse?.response?.results.length} bilder. Velger bilde nummer ${tilfeldigTall}`
    );
    const tilfeldigValgtBilde = unsplashResponse?.response?.results[tilfeldigTall];
    ...
    log.success(
        `Kjører bilde gjennom Sqip med følgende parameter: ${JSON.stringify(options, null, 2)}`
    );
    ...
    log.success(`Du kan se resultatet fra Sqip her: ${path.dirname(__filename)}/result.svg`);
    ...
    log.success(
        `Du har spart: ${regnUtBesparelseIProsent(originalStorrelse, nyStorrelse).toFixed(2)}%`
    );
}
